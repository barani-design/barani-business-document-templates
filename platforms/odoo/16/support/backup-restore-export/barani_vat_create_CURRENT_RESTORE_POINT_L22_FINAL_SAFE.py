# ============================================================================
# ACTION NAME : BARANI VAT REPORT — CREATE CURRENT RESTORE POINT L22 FINAL SAFE
...
#               accepted L22 final template can be restored later.
#
...
BACKUP_TAG = 'public_current_template'
...
ACTION_NAME = 'BARANI VAT REPORT — CREATE CURRENT RESTORE POINT L22 FINAL SAFE'
...
try:
    env.cr.execute('SAVEPOINT t0_l22_rp_ok')
    env.cr.execute('SELECT 1')
    env.cr.execute('RELEASE SAVEPOINT t0_l22_rp_ok')
    env.cr.execute('SAVEPOINT t0_l22_rp_fail')
    try:
        env.cr.execute('SELECT * FROM __barani_missing_table_for_rollback_probe__')
        env.cr.execute('RELEASE SAVEPOINT t0_l22_rp_fail')
    except Exception:
        env.cr.execute('ROLLBACK TO SAVEPOINT t0_l22_rp_fail')
        env.cr.execute('RELEASE SAVEPOINT t0_l22_rp_fail')
        try:
            env.invalidate_all()
...
lines.append('L22 FINAL MARKER CHECK')
checks = [
    ...
    ('customer/shipping email present', 'barani_customer_email' in vat_arch and 'barani_shipping_email' in vat_arch and 'Email: ' in vat_arch),
    ('invoice/SO note fallback present', 'barani_sale_order_note_fallback' in vat_arch and 'barani_note_source_orders' in vat_arch and 'sale_line_ids.order_id' in vat_arch),
...
for c in checks:
    ...
    if not c[1]:
        marker_bad = True
if marker_bad:
    lines.append('ERROR: L21 marker check failed. Refusing to snapshot as L22 final.')
    raise UserError('\n'.join(lines)[:90000])
lines.append('')
...
try:
    env.cr.execute('SAVEPOINT sp_vat_l22_restore_point')
    ...
    if cache_inv_method == 'env.invalidate_all':
        ...
        env.cache.invalidate()
    env.cr.execute('RELEASE SAVEPOINT sp_vat_l22_restore_point')
    ...
    lines.append('BACKUP COMPLETE. You can restore this L22 state with the companion restore script.')
except Exception as e:
    try:
        env.cr.execute('ROLLBACK TO SAVEPOINT sp_vat_l22_restore_point')
        env.cr.execute('RELEASE SAVEPOINT sp_vat_l22_restore_point')
    except Exception:
        pass
