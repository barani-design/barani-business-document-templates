# ============================================================================
# ACTION NAME : READ-ONLY: BARANI Delivery Note 2026 readiness probe v2
# MODEL       : Any model; selected records ignored unless they are stock.picking samples.
# ACTION TO DO: Execute Python Code
# PURPOSE     : Verify the Delivery Note 2026 report prerequisites, required
#               stock/product fields, existing stock.picking report actions,
#               and optional selected picking samples.
#
...
PAGE = 1
PAGE_SIZE = 80000
ACTION_NAME = 'READ-ONLY: BARANI Delivery Note 2026 readiness probe v2'
...
for model_name, field_names in checks:
    ...
    if model_name in env:
        ...
        for fname in field_names:
            ok = fname in flds
            lines.append('    %-30s %s' % (fname, 'YES' if ok else 'NO'))
            if not ok:
                missing_any = True
    else:
        missing_any = True
...
for model_name, field_names in optional:
    lines.append('  %s:' % model_name)
    if model_name in env:
        flds = env[model_name]._fields
        for fname in field_names:
            lines.append('    %-30s %s' % (fname, 'YES' if fname in flds else 'NO'))
    else:
        lines.append('    MODEL MISSING')
lines.append('')
