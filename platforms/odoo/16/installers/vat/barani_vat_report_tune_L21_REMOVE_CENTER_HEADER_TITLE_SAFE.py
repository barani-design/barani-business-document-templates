# ============================================================================
# ACTION NAME : BARANI VAT REPORT TUNE L21 — remove duplicate centered header title
#               (on L20 base; cosmetic header only; RI keeps payment ref; production label)
#
# L21 CHANGE (cosmetic only):
#   1. Remove the duplicate centered document title in the header.
#   2. Keep the right-side document type + number label only, e.g. Invoice: 20XXNNN.
...
#      (invoice_origin, e.g. Q20XXNNN) instead of the DPI's own number.
...
ACTION_NAME = 'BARANI VAT REPORT TUNE L21 — remove duplicate centered header title (RI keeps payment ref)'
...
RECEIVING_IBAN = '__RECEIVING_IBAN_DISPLAY__'
RECEIVING_IBAN_COMPACT = '__RECEIVING_IBAN_COMPACT__'
RECEIVING_BIC = '__RECEIVING_BIC__'
...
try:
    env.cr.execute('SAVEPOINT sp_vat_l21_install')
    paper_vals = {
    ...
    lines.append('PASS: read-back verification passed (native label, header IDs/EORI, official totals, fiscal-position display-table notes, Incoterms code/name, issued-by, credit-note reference/payment cleanup, Intrastat transport once, final table cleanup, L20 company-registration/shipping-column alignment, L19 address no-indent/gutter fix and duplicate-row cleanup, customer/shipping phone Tel prefix, RI Down payment reconciliation table/no duplicate gross rows, confirmed bank fallback, visibility groups, L15.2 bank/no-QR, layout wrapper).')

    env.cr.execute('RELEASE SAVEPOINT sp_vat_l21_install')
    lines.append('INSTALL COMPLETE.')
    lines.append('TEST: Print -> %r.' % VAT_REPORT_NAME)
    lines.append('Expected: L21 keeps L20/L19/L16g.3/L16h.1/L17 final behavior, removes the duplicate centered header title, keeps only the right-side document type + number label, adds Customer/Shipping phone numbers with Tel: prefix when available, prints an RI Down payment reconciliation table with negative base/VAT/total down-payment rows, removes the duplicate right-side down-payment rows, removes the red-band reconciliation note, and keeps the company tax-registration header block aligned with the Shipping Address column; settlement invoice right-side totals show Odoo official net invoice totals plus Balance Due and Currency, while the Down payment reconciliation table shows supply before down payments and each negative down-payment base/VAT/total row; DP invoices (20XXNNN-181) show "Subtotal (excl. VAT)" label, NO Balance Due row, and Payment Reference = Source order ref (Q20XXNNN); header dividers/white-space/issued-by/footer polish applied; Bank transfer band shows IBAN + SWIFT/BIC + Bank + Bank address; QR omitted; rounding note retained; report visible to Billing + Sales/Administrator.')
    lines.append('HYGIENE: reset APPLY=False / CONFIRM="", or archive/delete this server action.')
except Exception as e_apply:
    try:
        env.cr.execute('ROLLBACK TO SAVEPOINT sp_vat_l21_install')
        env.cr.execute('RELEASE SAVEPOINT sp_vat_l21_install')
        if cache_inv_method == 'env.invalidate_all':
            env.invalidate_all()
        else:
